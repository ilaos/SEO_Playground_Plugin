...
if ( ! is_admin() && ! wp_doing_ajax() && ! wp_doing_cron() && ! $almaseo_is_rest && ! ( defined( 'WP_CLI' ) && WP_CLI ) ) {
    ...
    if ( $almaseo_seo_conflict ) {
        ...
        if ( ! defined( 'ALMASEO_PLUGIN_VERSION' ) ) define( 'ALMASEO_PLUGIN_VERSION', '1.21.6' );
...
if (!defined('ALMASEO_PATH'))            define('ALMASEO_PATH', plugin_dir_path(__FILE__));
if (!defined('ALMASEO_URL'))             define('ALMASEO_URL', plugin_dir_url(__FILE__));
if (!defined('ALMASEO_PLUGIN_VERSION'))  define('ALMASEO_PLUGIN_VERSION', '1.21.5');
...
if (!defined('ALMASEO_DASHBOARD_URL'))   define('ALMASEO_DASHBOARD_URL', 'https://app.almaseo.com');
...
// Include Author Profile Fields (linked author Person schema — E-E-A-T)
if (file_exists(plugin_dir_path(__FILE__) . 'includes/admin/author-profile-fields.php')) {
    require_once plugin_dir_path(__FILE__) . 'includes/admin/author-profile-fields.php';
}
...
if (!function_exists('almaseo_display_content_reminders')) {
    ...
    jQuery(document).on('click', '.almaseo-content-reminder .notice-dismiss', function() {
        ...
        jQuery.post(ajaxurl, {
            ...
            nonce: <?php echo wp_json_encode(wp_create_nonce('almaseo_dismiss_content_reminder')); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Intentional JSON output ?>,
            post_id: postId
        });
    });
    </script>
...
if (!function_exists('almaseo_api_auth_check')) {
    function almaseo_api_auth_check($request) {
        ...
        if (strpos($auth_header, 'Basic ') === 0) {
            ...
            // Accept any authenticated user with edit_posts capability (administrator/editor)
            // This allows both AlmaSEO-generated and manually-created app passwords to work
            if (user_can($user, 'edit_posts')) {
                return true;
            }

            return new WP_Error('invalid_auth', 'Insufficient permissions.', array('status' => 403));
        }
...
add_action('admin_init', function() {
    if (get_option('almaseo_playground_do_activation_redirect', false)) {
        ...
        if (!isset($_GET['activate-multi'])) {
            ...
            // plugins, run wizard, import). The setup wizard is still
            // reachable via its menu submenu link for users who want it.
