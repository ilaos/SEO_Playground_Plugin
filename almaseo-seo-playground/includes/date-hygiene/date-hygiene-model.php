<?php
/**
 * Date Hygiene Scanner – Data Model
 *
 * Static CRUD class for the date-hygiene table.
 *
 * @package AlmaSEO
 * @since   7.3.0
 */
...
class AlmaSEO_Date_Hygiene_Model {
    ...
    private static function table() {
        global $wpdb;
        return $wpdb->prefix . 'almaseo_date_hygiene';
    }
    ...
    /**
     * Distinct post count with open findings, grouped by severity.
     *
     * @return array { total_posts: int, high: int, medium: int, low: int }
     */
    public static function get_post_summary() {
        global $wpdb;
        $table = self::table();
        ...
        foreach ( $rows as $r ) {
            if ( isset( $summary[ $r->severity ] ) ) {
                $summary[ $r->severity ] = (int) $r->cnt;
            }
            $summary['total_posts'] += (int) $r->cnt;
        }
