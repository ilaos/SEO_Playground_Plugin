...
class Alma_Provider_Posts {
    ...
    /**
     * Build JOIN clauses for exclusion filtering
     */
    private function build_exclude_joins() {
        global $wpdb;
        $joins = '';

        $exclude = $this->settings['exclude'] ?? array();
        $excluded_terms = $exclude['taxonomies'] ?? array();

        // Only add taxonomy join if we have terms to exclude
        if (!empty($excluded_terms)) {
            $term_ids = implode(',', array_map('absint', $excluded_terms));
            $joins .= " LEFT JOIN (
                SELECT DISTINCT tr.object_id
                FROM {$wpdb->term_relationships} tr
                INNER JOIN {$wpdb->term_taxonomy} tt ON tr.term_taxonomy_id = tt.term_taxonomy_id
                WHERE tt.term_id IN ({$term_ids})
            ) excluded_terms ON excluded_terms.object_id = p.ID ";
        }
    ...
    /**
     * Build WHERE clauses for exclusion filtering
     */
    private function build_exclude_where() {
        global $wpdb;
        $where = '';
        ...
        $older_than_years = absint($exclude['older_than_years'] ?? 0);

        // Exclude posts in specified taxonomies
        if (!empty($excluded_terms)) {
            $where .= " AND excluded_terms.object_id IS NULL ";
        }

        // Exclude posts by specified authors
        if (!empty($excluded_authors)) {
            $author_ids = implode(',', array_map('absint', $excluded_authors));
            $where .= " AND p.post_author NOT IN ({$author_ids}) ";
        }
